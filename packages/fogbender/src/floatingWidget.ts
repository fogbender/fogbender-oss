...
export function createFloatingWidget(rootEl: HTMLElement, url: string, token: Token) {
  ...
  const badgesCounter = document.createElement("div");
  ...
  button.appendChild(badgesCounter);
  ...
  on<{ badges: { [roomId: string]: Badge } }>(rootEl, "fogbender.badges", e => {
    ...
    badgesCounter.innerHTML = unreadCount.toString();
    badgesCounter.style.display = unreadCount > 0 ? "block" : "none";
  });
}
