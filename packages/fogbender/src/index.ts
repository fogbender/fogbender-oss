  ...
  const fogbender: Fogbender & { _privateData: any } = {
    ...
    async setToken(token) {
      ...
      if (state.token) {
        state.token = {
          ...state.token,
          versions: { ...state.token.versions, ...state.versions, fogbender: "0.0.2" },
        };
      }
      updateConfigured();
      return fogbender;
    ...
    async createFloatingWidget(style: { verbose?: boolean } = {}) {
      if (!state.url) {
      ...
      const cleanup = createFloatingWidget(state, openWindow, style);
      return cleanup;
    },
